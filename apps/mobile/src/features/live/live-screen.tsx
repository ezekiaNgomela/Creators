...
export function LiveScreen() {
  const { comments, liveIndex, liveRooms, openLive, selectedLiveRoom, closeLive, addLiveComment } = useApp();
  ...
  if (!selectedLiveRoom) {
    return (
      <ScrollView contentContainerStyle={{ padding: spacing.lg, gap: spacing.lg, paddingBottom: 120 }} contentInsetAdjustmentBehavior="automatic" style={{ flex: 1, backgroundColor: palette.bg }}>
        <LinearGradient colors={["#161f30", "#0b0f17"]} style={{ borderRadius: 34, padding: spacing.xl, minHeight: 320, justifyContent: "flex-end" }}>
          <Text style={{ color: palette.accent, fontWeight: "900", textTransform: "uppercase", letterSpacing: 1.2 }}>Live landing</Text>
          <Text style={{ color: palette.text, fontWeight: "900", fontSize: 38, marginTop: spacing.sm }}>Favorite streams</Text>
          <Text style={{ color: palette.textMuted, lineHeight: 22, marginTop: spacing.md }}>
            Tap any room to move into the live watch flow with stream chat, reactions, and the focused player view.
          </Text>
        </LinearGradient>
        <Text style={liveStyles.sectionTitle}>Live now</Text>
      ...
      <View style={{ overflow: "hidden", borderRadius: radius.xl, borderWidth: 1, borderColor: palette.stroke, backgroundColor: palette.panel }}>
        <Pressable onPress={closeLive}>
          ...
          <View style={{ position: "absolute", left: spacing.lg, right: spacing.lg, bottom: spacing.lg }}>
            <Text style={{ color: palette.accent, fontWeight: "900", textTransform: "uppercase" }}>Now watching</Text>
